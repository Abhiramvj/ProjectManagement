        ...
        <div class="p-4 sm:p-6 lg:p-8">
            ...
            <div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                        <label for="employee_search" class="block text-sm font-medium text-gray-700">
                            Employee
                        </label>
                        <input
                            v-model="search"
            ...
            <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="user in calendarData.data" :key="user.id" class="hover:bg-gray-50">
                                ...
                                >
                                    <div>
                                        ...
                                        <div v-if="user.teams" class="text-xs text-gray-500">
                                            {{ user.teams }}
                                        </div>
                                    </div>
                                </td>
                                <td v-for="date in dateRange" :key="date" class="py-2 text-center">
                                    <div class="flex h-full items-center justify-center">
                                        <div
                                            v-if="user.daily_statuses[date]?.status === 'Leave' && user.daily_statuses[date]?.details"
                                            ...
                                            :style="{
                                                backgroundColor: user.daily_statuses[date].details.color,
                                            }"
                                        >
                                            {{ user.daily_statuses[date].details.code }}
                                        </div>
                                        <div
                                            v-else-if="user.daily_statuses[date]?.status === 'Leave'"
                                        ...
                                        <div
                                            v-else-if="user.daily_statuses[date]?.status === 'Working'"
                                        ...
                                        <div
                                            v-else-if="user.daily_statuses[date]?.status === 'Future'"
                                            class="text-sm text-gray-300"
                                            title="Future date"
                                        >
                                            ○
                                        </div>
                                        <div class="text-gray-400" title="Weekend/Holiday">
                                            ·
                                        </div>
                                    </div>
                                </td>
                            ...
                            <tr v-if="calendarData.data.length === 0">
                                <td :colspan="dateRange.length + 1" class="py-10 text-center text-gray-500">
                                    <div v-if="filters.absent_only">
                                        ...
                                        <div class="mt-1 text-sm text-gray-400">
                                            Try expanding the date range or uncheck "Show absent only"
                                        </div>
                                    </div>
                                    <div v-else>
                ...
                <div v-if="calendarData.total > 0" class="flex items-center justify-between border-t border-gray-200 p-4">
            ...
            <div class="mt-4 rounded-lg bg-gray-50 p-3">
                ...
                <div class="flex flex-wrap gap-4 text-xs text-gray-600">
                    <div class="flex items-center gap-1">
                        <span class="text-lg text-green-500">✓</span><span>Present</span>
                    </div>
                    <template v-if="authUserRole == 'employee'">
                        <div class="flex items-center gap-1">
                            <span class="text-lg text-red-500">✗</span><span>Absent</span>
                        </div>
                    </template>
                    <template v-if="authUserRole !== 'employee'">
                        <div class="flex items-center gap-1">
                            <div class="flex h-4 w-4 items-center justify-center rounded bg-red-400 text-xs text-white">
                                A
                            </div><span>Annual Leave</span>
                        </div>
                        <div class="flex items-center gap-1">
                            <div class="flex h-4 w-4 items-center justify-center rounded bg-orange-400 text-xs text-white">
                                S
                            </div><span>Sick Leave</span>
                        </div>
                        <div class="flex items-center gap-1">
                            <div class="flex h-4 w-4 items-center justify-center rounded text-xs text-white" style="background-color: #9C27B0;">
                                P
                            </div><span>Personal Leave</span>
                        </div>
                    </template>
                    <div class="flex items-center gap-1">
                        <span class="text-sm text-gray-300">○</span><span>Future date</span>
                    </div>
                    <div class="flex items-center gap-1">
                        <span class="text-gray-400">·</span><span>Weekend/Holiday</span>
                    </div>
                </div>
            </div>
        </div>
