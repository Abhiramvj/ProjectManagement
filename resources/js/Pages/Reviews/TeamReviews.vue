...
export default {
  ...
  methods: {
    ...
    openCreateReviewModal(member, month = null, year = null) {
      this.selectedMemberForReview = member;
      const now = new Date();
      this.reviewMonth = month || now.getMonth() + 1;
      this.reviewYear = year || now.getFullYear();
      this.showReviewModal = true;
    },
    // Updated goToCreateReview to open modal instead of navigating
    goToCreateReview(employeeId, month, year) {
      const member = this.members.find((m) => m.id === employeeId);
      if (member) {
        this.openCreateReviewModal(member, month, year);
      }
    },
    ...
    // Handle modal close after submit or cancel
    handleModalClose() {
  ...
  console.log('Modal submitted event received');
  this.showReviewModal = false;
  this.selectedMemberForReview = null;
},
  ...
  try {
    await this.$inertia.post('employee.review.store');
    this.$emit('submitted'); 
  } catch (error) {
    // handle error
  } finally {
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-7">
        ...
        >
          <div class="flex items-center gap-4 mb-2">
            <div
              class="bg-gray-100 rounded-full h-12 w-12 flex items-center justify-center text-xl font-bold text-gray-700 border border-gray-200"
            >
          ...
          <div class="flex items-center justify-between text-sm mb-2">
            ...
            <span
              :class="[
                ...
                member.status === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600',
              ]"
            >
              {{ member.status }}
            </span>
          ...
          <div
            ...
            :class="{ 'text-green-600': member.performance > 0, 'text-gray-400': !member.performance }"
          >
          ...
          <div class="text-gray-500 text-sm mb-4 flex items-center gap-2">
            <svg
              class="w-4 h-4 inline-block text-gray-400"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
              viewBox="0 0 24 24"
...
>
  <div
    class="bg-white rounded-xl shadow-lg max-w-4xl w-full mx-4 p-6 overflow-auto max-h-[90vh]"
    @click.stop  
  >
<CreateReview
  :employee-id="selectedMemberForReview?.id"
  :categories="categories"
  :review-month="reviewMonth"
  :review-year="reviewYear"
  cancelRoute="reviews.team"
  submitRouteName="employee.review.store"
